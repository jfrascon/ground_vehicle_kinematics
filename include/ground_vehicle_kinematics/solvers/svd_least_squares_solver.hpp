...
namespace ground_vehicle_kinematics
{
  ...
  class SvdLeastSquaresSolverConfig
  {
     ...
     * @param relative_singular_value_threshold Relative threshold used to discard weak singular directions.
     * @param tikhonov_lambda Tikhonov damping factor.
     */
    SvdLeastSquaresSolverConfig(double relative_singular_value_threshold = 1e-6, double tikhonov_lambda = 1e-4);
   ...
   *
   * The factorization is built in the constructor or in @ref reset. Calls to @ref solve reuse that precomputed
   * result.
   */
  class SvdLeastSquaresSolver
  {
     ...
     */
    explicit SvdLeastSquaresSolver(const Eigen::MatrixXd& A, const SvdLeastSquaresSolverConfig& cfg = {});
     ...
     * @throws std::invalid_argument If the input dimensions do not match the current matrix dimensions.
     */
