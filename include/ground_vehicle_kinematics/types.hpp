...
namespace ground_vehicle_kinematics
{
  ...
  class Limits
  {
    public:
       ...
       */
      explicit Limits(const double lower = 0.0, const double upper = 0.0): lower_{lower}, upper_{upper}
      {
        validate();
      }
       ...
       * @param tolerance Numerical tolerance used to absorb floating-point roundoff at the
       * boundaries.
      ...
      bool contains(const double value, const double tolerance = 1e-9) const
      {
        ...
        // Use a small tolerance around the limits.
        // This avoids rejecting values that only differ because of floating-point roundoff.
  ...
  class Position2D
  {
    public:
       ...
       */
      explicit Position2D(const double x = 0.0, const double y = 0.0): x_{x}, y_{y}
      {
        validate();
      }
  ...
  class SteeringJoint: public Joint
  {
    ...
    private:
       ...
       *
       * The kinematic model assumes that steering needs at most one full turn to either side of
       * its nominal orientation. That produces the bound [-2pi, 2pi].
       *
       * A real actuator may rotate farther, but its configuration must use the equivalent interval
       * within [-2pi, 2pi] that is meaningful for this model.
       */
  ...
  /**
   * @brief Wheel model containing planar geometry, radius, and its rotation joint.
   */
  class Wheel
  {
    public:
  ...
  class WheelCommand
  {
    public:
      ...
      WheelCommand(const Wheel& wheel, const double wheel_ang_vel, const int64_t timestamp_ns = 0):
        ...
        rotation_joint_command_{wheel.rotation_joint().name(),
                                std::clamp(wheel_ang_vel,
  ...
  class SteerableWheelCommand: public WheelCommand
  {
    public:
      ...
      SteerableWheelCommand(const SteerableWheel& wheel,
                            ...
                            const int64_t timestamp_ns = 0):
        // The WheelCommand base must be initialized, but there is not enough information yet
      ...
      {
        // If linear speed is almost zero, stop the wheel and keep its reported steering angle.
        ...
        // Several angle and speed pairs can produce the same rolling direction.
        // Build those candidates, then select the one that best fits the current state and limits.
    ...
    private:
      ...
      // create_candidate_solutions explains why the solver always creates five candidates.
      static constexpr std::size_t candidate_count_{5};

      /**
       * @brief Generate equivalent steerable-wheel solutions for one kinematic solution.
      ...
      static std::array<CandidateSolution, candidate_count_> create_candidate_solutions(const double steering_angle,
                                                                                        const double wheel_ang_vel)
      {
        // Adding or subtracting 2pi preserves both direction and wheel angular velocity.
        // Adding or subtracting pi preserves direction when the wheel angular velocity is reversed.
      ...
      /**
       * @brief Compute the distance from a candidate angle to the current steering angle.
       ...
       * @param wheel_state Current steerable-wheel state.
       * @return Best candidate solution. Non-clamped solutions are preferred. Within each group,
       * the closest candidate to the current steering angle is selected.
       */
      static CandidateSolution select_candidate_solution(
        ...
        const SteerableWheelState& wheel_state)
      {
        // Separate candidates that satisfy the steering limits from candidates that need clamping.
        // Prefer the first group whenever it is not empty.
        // In the selected group, choose the candidate closest to the current steering angle.
        ...
        for(std::size_t i{0}; i < candidate_solutions.size(); ++i)
        {
          ...
          {
            // Project an invalid steering angle onto the nearest allowed boundary.
            // Keep its wheel angular velocity.
            // Selection later decides whether it is the best fallback.
        ...
        {
          // Use clamped candidates only when every equivalent solution violates the limits.
