...
\begin{itemize}
    \item \textbf{Restricción de rodadura:} la rueda solo puede moverse en la dirección de rodadura.
    \item \textbf{Restricción de no deslizamiento lateral (rueda convencional):} la velocidad de contacto entre rueda y suelo en la dirección perpendicular a la rodadura es nula.
    \item \textbf{Ruedas omnidireccionales/suecas:} esta restricción lateral no aplica de forma estricta, ya que los rodillos permiten movimiento lateral pasivo.
\end{itemize}
...
El término \(d\) representa la distancia desde el SC del robot hasta el punto de contacto de la rueda con el suelo.
El término \(\alpha\) representa el ángulo que forma el eje \(\ejeXR\) con el vector de posición de la rueda, \(\vect{d}\), y por cómo se posiciona la rueda, es también el ángulo que forma el eje \(\ejeXR\) con el eje \(\ejeYO\). El eje \(\ejeXO\) es perpendicular al eje \(\ejeYO\) y su orientación con respecto al eje \(\ejeXR\) es \(\alpha - \frac{\pi}{2}\).
...
\begin{figure}[H]
    ...
    \caption{Posicionamiento 2D de una rueda respecto al SC del robot.}
    \label{fig:wheel-placement}
\end{figure}

A continuación, se considera un ángulo \(\beta\) que permite orientar la rueda de la manera deseada, como se muestra en la figura \ref{fig:wheel-orientation}.
...
\begin{figure}[H]
    ...
    \caption{Orientación de la rueda mediante el ángulo \(\beta\).}
    \label{fig:wheel-orientation}
\end{figure}

El SC \(\frameB\) se denomina \textbf{SC base de la rueda}, y su posición y orientación se mantiene constante respecto del SC \(\frameR\). En ruedas no orientables, el eje \(\ejeXB\) determina la dirección de rodadura de la rueda y el eje \(\ejeYB\) es el eje de giro de la rueda. En ruedas orientables, el SC fijo a la rueda, \(\frameWi\), va cambiando su orientación (ángulo \(\theta\)) respecto al SC \(\frameB\) y sólo cuando \(\theta=0\) coinciden. Ver figura \ref{fig:steerable-wheel}.
...
\begin{figure}[H]
    ...
    \caption{Velocidades lineales y angulares relevantes para la rueda.}
...
\begin{figure}[H]
    ...
    \caption{Rueda convencional orientable rotando un ángulo \(\theta\) respecto al SC \(\frameB\) para conseguir la dirección de rodadura adecuada.}
...
El vector \({}^\frameR \vect{V}_{i}\) representa la velocidad lineal de la rueda \(i\) en el SC \(\frameR\).
Para aplicar las restricciones de movimiento a cada rueda, se debe expresar su velocidad en su SC base, \(\frameBi\), \({}^{\frameBi} V_{i}\).
...
donde la matriz de rotación \({}^{\frameBi}  R(\psi_i)_3\) representa la orientación del SC \(\frameR\) en el SC \(\frameBi\), y se usa para convertir vectores expresados en el SC \(\frameR\) al SC base de la rueda \(i\), \(\frameBi\).
...
La velocidad \({}^{\frameBi} \vect{V}_i\) se puede expresar, de forma general, como:
...
siendo \(v_{i\parallel}\) la componente de velocidad en la dirección de rodadura, \(\ejeXWi\), y \(v_{i\perp}\) la componente de velocidad perpendicular a la dirección de rodadura (dicho de otro modo, la velocidad lateral de la rueda, paralela al eje \(\ejeYWi\)).
...
Conocido el vector de velocidad del chasis \({}^\frameR \vect{V}_r\), se obtiene la velocidad de cada rueda \(i\) en su SC base \(\frameBi\), \({}^{\frameBi}\vect{V}_i = [v_{i\parallel}, v_{i\perp}]^{\trans}\), aplicando la transformación de coordenadas de la ecuación \ref{eq:wheel-constraints-final}.

Dado que cada rueda es orientable, se puede calcular el ángulo de orientación \(\theta_i\) que debe adoptar cada rueda, respecto del SC base \(\frameBi\), para que su dirección de rodadura quede alineada con el vector de velocidad \({}^{\frameBi} \vect{V}_i\)
...
Conocido el ángulo de orientación \(\theta_i\) y la velocidad angular \(\omega_i\) de cada rueda \(i\), se puede calcular el vector de velocidad de cada rueda en su SC base \(\frameBi\), \({}^{\frameBi}\vect{V}_i = [v_{i\parallel}, v_{i\perp}]^{\trans}\). Conocidos todos los vectores \({}^{\frameBi}\vect{V}_i\), se construye el vector \(\mathbf{b} = [v_{1\parallel}, v_{1\perp}, \ldots, v_{N\parallel}, v_{N\perp}]^{\trans}\) y se resuelve el sistema lineal \(A\mathbf{x} = \mathbf{b}\) para obtener la velocidad del chasis \(\mathbf{x} = [{}^\frameR V_{r,x}, {}^\frameR V_{r,y}, \omega_{r,z}]^{\trans}\).

A partir de la ecuación \ref{eq:tsd-inverse-kinematics-speed-norm} se obtienen las velocidades \(v_{i\parallel}\) y \(v_{i\perp}\):
...
\begin{equation}
    \begin{aligned}
        ...
        \label{eq:tsd-direct-kinematics-v_parallel-v_perp}
    \end{aligned}
\end{equation}
...
El vector \(\mathbf{x}\) se calcula aplicando la pseudoinversa de \(A\), \(A^+\), al vector \(\mathbf{b}\) como se explicó en la Sección \ref{sec:general-considerations}.
