...
namespace ground_vehicle_kinematics
{
  namespace
  {
    FourSwerveKinematicsSolverConfig make_four_swerve_solver_config()
    {
      constexpr double wheel_radius{0.117494};
      constexpr double dist{0.4366337137693332};
      constexpr double alpha_0{0.5683101050173379};
      constexpr double alpha_1{-0.5683101050173379};
      constexpr double alpha_2{2.5732825485724553};
      constexpr double alpha_3{-2.5732825485724553};
      constexpr double beta_0{(M_PI / 2.0) - alpha_0};
      constexpr double beta_1{(M_PI / 2.0) - alpha_1};
      constexpr double beta_2{(M_PI / 2.0) - alpha_2};
      constexpr double beta_3{(M_PI / 2.0) - alpha_3};

      return FourSwerveKinematicsSolverConfig{std::array<SteerableWheelConfig,
                                                         4>{SteerableWheelConfig{wheel_radius,
                                                                                 dist,
                                                                                 alpha_0,
                                                                                 beta_0,
                                                                                 "wheel_0",
    ...
    FourSwerveKinematicsSolver make_four_swerve_solver()
    {
      return FourSwerveKinematicsSolver{make_four_swerve_solver_config()};
    }

    std::array<SteerableWheelState, 4> build_wheel_states_from_twist(const FourSwerveKinematicsSolverConfig& solver_cfg,
                                                                     const Twist& twist,
      ...
      for(std::size_t i{0}; i < wheel_cfgs.size(); ++i)
      {
        ...
        wheel_states[i] = SteerableWheelState{wheel_cfg.wheel_name(),
                                              RotationJointState{wheel_cfg.rotation_joint_name(),
                                                                 0.0,
                                                                 std::hypot(wheel_vel_x, wheel_vel_y) /
                                                                   wheel_cfg.radius(),
                                                                 timestamp_ns},
  ...
  TEST(FourSwerveKinematicsSolverTest, ReturnsWheelCommandsInDocumentedWheelOrder)
  {
    FourSwerveKinematicsSolver solver{make_four_swerve_solver()};
    const auto solver_cfg{make_four_swerve_solver_config()};
    ...
    EXPECT_EQ(wheel_cmds[3].get().wheel_name(), "wheel_3");
  }

  TEST(FourSwerveKinematicsSolverTest, ComputesTwistFromRawWheelData)
  {
    FourSwerveKinematicsSolver solver{make_four_swerve_solver()};
    const auto solver_cfg{make_four_swerve_solver_config()};
  ...
  TEST(FourSwerveKinematicsSolverTest, RoundTripsJointCommandsAndTwist)
  {
    FourSwerveKinematicsSolver solver{make_four_swerve_solver()};
    const auto solver_cfg{make_four_swerve_solver_config()};
